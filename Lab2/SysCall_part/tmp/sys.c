...
SYSCALL_DEFINE1(my_processinfo_top, struct my_processinfo __user *, info){
	...
	for_each_process(task) {
		...
		// PID
		InSyscall_info[process_cnt].pid = task->pid;
		
		// Command
		for(i = 0; i < TASK_COMM_LEN ; i++) {
			InSyscall_info[process_cnt].comm[i] = task->comm[i];
		}
		
		// State
		InSyscall_info[process_cnt].state = (task->state == 0) ? 1:0;
		
		// Runtime
		InSyscall_info[process_cnt].process_runtime = task->se.sum_exec_runtime;
		
		// Start time
		InSyscall_info[process_cnt].process_startime = task->start_time;
